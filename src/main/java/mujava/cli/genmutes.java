...
//		file_list = new String[listOfFilesInSession.length];
		...
		if (jct.getParameters() != null && jct.getParameters().size() > 1) {
			file_list = new File[jct.getParameters().size() - 1];
			int count = 0;
			int z = 0;
			for (String fileName : jct.getParameters()) {
				String strFileName = fileName.replace(".", "/");
				if (count != 0) {					
					for (int i = 0; i < files.size(); i++) {
						if (files.get(i).getPath().contains(strFileName)) {
							file_list[z++] = files.get(i);
						}
					}
				}
				count++;
...
//				String temp = file.substring(0, file.length() - ".java".length());
				...
				}
				{
					...
					File folder = new File(
							MutationSystem.MUTANT_HOME + "/" + class_name + "/" + MutationSystem.CM_DIR_NAME);
					File[] listOfMethods = folder.listFiles();
